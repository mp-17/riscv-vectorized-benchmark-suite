		...
		for (k=0; k<(1+box[l].nn); k++)
		{
			...
			for (i=0; i<NUMBER_PAR_PER_BOX; i=i+1){
				...
				for (j=0; j<NUMBER_PAR_PER_BOX; j+=gvl){
					...
					xr2    = _MM_ADD_VF_f32(xrB_v, rA[i].v, gvl);
					xDOT   = _MM_MUL_VF_f32(xrB_x, rA[i].x, gvl);
					xDOT   = _MM_MACC_VF_f32(xDOT,rA[i].y,xrB_y,gvl);
					xDOT   = _MM_MACC_VF_f32(xDOT,rA[i].z,xrB_z,gvl);
					xr2    = _MM_SUB_f32(xr2, xDOT,gvl);
					//u2 = a2*r2;
					xu2    = _MM_MUL_VF_f32(xr2, a2, gvl);
					//vij= exp(-u2);
